...
export default function PriceCardsData() {
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-8 w-full mt-14">
        {data.length > 0 ? (
          data.map((price) => (
            <div
              key={price.id}
      ...
      {/* {data.length > 0 && (
        ...
        >
          {data.map((price) => (
            <SwiperSlide
              key={price.id}
              className="bg-[#FFE2E5] rounded-[220px] px-8 py-12 flex flex-col items-center text-center w-[318px] min-h-[612px]"
            >
              <h3 className="text-[34px] font-semibold mb-10">{price.name}</h3>
              {price.name === "Free" ? (
                <p className="free-tier text-[68px] font-bold mb-[44px]">
